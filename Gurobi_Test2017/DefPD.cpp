...
void DefPD::resolveStaticDefPenetration() {
	...
	pTet = pTetAll.pTets2[minOptIndex];
	optimized = true;
	numOpt++;
...
void DefPD::optStaticFace(int fIndex, int pairIndex) {
	...
	try {
		...
		pTetAll.normal[pairIndex] = normal;
		// Add constraints: 
		// n·(p0-s0)>=0
		// n·(p1-s0)>=0
		// n·(p2-s0)>=0
		...
		initTet(pTetAll.pTets2[pairIndex], 
...
void DefPD::optEdgeEdge(int sIndex, int dIndex, int pairIndex) {
	...
	try {
		...
		switch (dIndex) {
		case 0:initTet(pTetAll.pTets2[pairIndex], p0, p1, p2, p3); break;//dIndex=0: p0=v0 p1=v1 p2=v2 p3=v3
		case 1:initTet(pTetAll.pTets2[pairIndex], p0, p2, p1, p3); break;//dIndex=1: p0=v0 p1=v2 p2=v1 p3=v3
		case 2:initTet(pTetAll.pTets2[pairIndex], p0, p2, p3, p1); break;//dIndex=2: p0=v0 p1=v3 p2=v1 p3=v2
		case 3:initTet(pTetAll.pTets2[pairIndex], p2, p0, p1, p3); break;//dIndex=3: p0=v1 p1=v2 p2=v0 p3=v3
		case 4:initTet(pTetAll.pTets2[pairIndex], p2, p0, p3, p1); break;//dIndex=4: p0=v1 p1=v3 p2=v0 p3=v2
		case 5:initTet(pTetAll.pTets2[pairIndex], p2, p3, p0, p1); break;//dIndex=5: p0=v2 p1=v3 p2=v0 p3=v1
		...
		if (nDecided == false) {
			...
			if (pTetAll.optValue[pairIndex] > model_second.get(GRB_DoubleAttr_ObjVal)) {
				...
				switch (dIndex) {
				case 0:initTet(pTetAll.pTets2[pairIndex], p0_, p1_, p2_, p3_); break;//dIndex=0: p0=v0 p1=v1 p2=v2 p3=v3
				case 1:initTet(pTetAll.pTets2[pairIndex], p0_, p2_, p1_, p3_); break;//dIndex=1: p0=v0 p1=v2 p2=v1 p3=v3
				case 2:initTet(pTetAll.pTets2[pairIndex], p0_, p2_, p3_, p1_); break;//dIndex=2: p0=v0 p1=v3 p2=v1 p3=v2
				case 3:initTet(pTetAll.pTets2[pairIndex], p2_, p0_, p1_, p3_); break;//dIndex=3: p0=v1 p1=v2 p2=v0 p3=v3
				case 4:initTet(pTetAll.pTets2[pairIndex], p2_, p0_, p3_, p1_); break;//dIndex=4: p0=v1 p1=v3 p2=v0 p3=v2
				case 5:initTet(pTetAll.pTets2[pairIndex], p2_, p3_, p0_, p1_); break;//dIndex=5: p0=v2 p1=v3 p2=v0 p3=v1
				default: cout << "[EdgeEdge Result]Wrong Edge Index \n" << endl; break;
				}
			}
		}
...
void DefPD::fprintV3(ofstream &fp,vec3 v) {
	fp <<  v.x << "," << v.y << "," << v.z  ;
...
void DefPD::printResult(int pairIndex) {
	...
	tet pTet = pTetAll.pTets2[pairIndex];
...
//}
void DefPD::separatingPlaneCalculation(vec3 faceVrtx[3], vec3 vrtx, vec3 *normal, double *d) {
	//compute the normal vector and constant of the plane equation for 3 vertices
	glm::vec3 v01 = faceVrtx[1] - faceVrtx[0];
...
void DefPD::writeCSVHead(string fileName) {

	ofstream output(fileName+".csv");
	
	output << " , INPUT,,,,,,,,OUTPUT,,,,,,,," << endl;
	output << "No. , StaticTet,,,,RestTet,,,,DeformedTet,,,,PD(obj. norm),Opt. Time,Sep. Index,Sep. Pair,Sep. Normal,,, Volume Before, Volme After"<< endl;

	output.close();
}


//	"no , s0,x,y,z,r0,x,y,z,p0,x,y,z,pd,time,pd index,normal.x,normal.y,normal.z,volume";

void DefPD::writeCSV(string fileName) {
	ofstream output(fileName + ".csv", ios::app);
	string pair;
	if (minOptIndex < 4) {
		pair=("Static Face" + to_string(minOptIndex));
	}
	else if (minOptIndex < 8) {
		pair=("Deforming Face" + to_string(minOptIndex - 4));
	}
	else if (minOptIndex < 44) {
		pair = "SE" + to_string((minOptIndex - 8) / 6) + " DE" + to_string((minOptIndex - 8) % 6);
	} 
	output << numOpt << ",s0,";
	fprintV3(output, sTet.vertex[0]);
	output << ",r0,";
	fprintV3(output, rTet.vertex[0]);
	output << ",p0,";
	fprintV3(output, pTet.vertex[0]);
	output << ",";
	...
	fprintV3(output, pTetAll.normal[minOptIndex]);
	output << "," << rVolume << 
			  "," << calculateTetVolume(pTet) << endl;

	output << ",s1,";
	fprintV3(output, sTet.vertex[1]);
	output << ",r1,";			 
	fprintV3(output, rTet.vertex[1]);
	output << ",p1,";			 
	fprintV3(output, pTet.vertex[1]);
	output << endl;
	output << ",s2,";
	fprintV3(output, sTet.vertex[2]);
	output << ",r2,";			
	fprintV3(output, rTet.vertex[2]);
	output << ",p2,";			
	fprintV3(output, pTet.vertex[2]);
	output << endl;
	output << ",s3,";
	fprintV3(output, sTet.vertex[3]);
	output << ",r3,";			 
	fprintV3(output, rTet.vertex[3]);
	output << ",p3,";			 
	fprintV3(output, pTet.vertex[3]);
	output << endl;
	output.close();
}
